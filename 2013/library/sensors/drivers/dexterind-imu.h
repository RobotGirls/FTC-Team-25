...
/*
 * $Id: dexterind-imu.h 112 2012-10-04 17:44:38Z xander $
 */
...
bool DIMUconfigAccel(tSensors link, ubyte range) {
  switch(range) {
	...
	if (!writeI2C(link, DIMU_I2CRequest[0]))     // (Port 1, Message Array, Reply Size)
	  return false;
...
float DIMUreadAccelAxis8Bit(tSensors link, ubyte axis){
  int sensorReading = 0;
	...
	if (!writeI2C(link, DIMU_I2CRequest[0], DIMU_I2CReply, 1))
	  return 0;
...
bool DIMUsetAccelAxisOffset(tSensors link, ubyte drift_reg, ubyte drift_LSB, ubyte drift_MSB){
  ...
  return writeI2C(link, DIMU_I2CRequest[0]);
}
...
float DIMUreadAccelAxis10Bit(tSensors link, ubyte axis, bool calibrate){
  ...
  if (!writeI2C(link, DIMU_I2CRequest[0], DIMU_I2CReply, 2))
    return 0;
