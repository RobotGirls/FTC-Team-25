...
@Autonomous(name = "Lilac Strafe Auto", group = "Team 25")
public class LilacStrafeAuto extends Robot {
   ...
   // private FourWheelDirectDrivetrain drivetrain;
    ...
    private LilacStrafeAuto.RobotPosition robotPosition;
    ...
    public void handleEvent(RobotEvent e) {
        if (e instanceof GamepadTask.GamepadEvent) {
            ...
            switch (event.kind) {
                case BUTTON_A_DOWN:
                    selectPosition(LilacStrafeAuto.RobotPosition.CRATER);
                    robotPosition = RobotPosition.CRATER;
                    positionItem.setValue("Crater");
                    setMarkerPath();
                    break;
                case BUTTON_Y_DOWN:
                    selectPosition(LilacStrafeAuto.RobotPosition.MARKER);
    ...
    private void selectPosition(LilacStrafeAuto.RobotPosition choice)
    {
        if (choice == RobotPosition.CRATER) {
            robotPosition = LilacStrafeAuto.RobotPosition.CRATER;
            RobotLog.i("506 Position: CRATER");

        } else {
            robotPosition = LilacStrafeAuto.RobotPosition.MARKER;
    ...
    private void setMarkerPath()
    {
        switch (robotPosition) {
            case CRATER:
                RobotLog.i("506 Set Crater path");
                unlatchScan.stop();
                unlatchScan.addSegment(DeadReckonPath.SegmentType.SIDEWAYS, 6, STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.STRAIGHT, 1, -STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.TURN, 3, -TURN_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.STRAIGHT, 5, -STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.SIDEWAYS, 8, STRAIGHT_SPEED);

                scoreMarker.stop();
                scoreMarker.addSegment(DeadReckonPath.SegmentType.STRAIGHT, 22, -0.6);
                break;
            case MARKER:
                RobotLog.i("506 Set Marker path");
                unlatchScan.stop();
                unlatchScan.addSegment(DeadReckonPath.SegmentType.SIDEWAYS, 6, STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.STRAIGHT, 1, -STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.TURN, 5, -TURN_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.STRAIGHT, 6, -STRAIGHT_SPEED);
                unlatchScan.addSegment(DeadReckonPath.SegmentType.SIDEWAYS, 9.5, STRAIGHT_SPEED);
    ...
    {
        RobotLog.i("506 Deciding whether to score marker");
