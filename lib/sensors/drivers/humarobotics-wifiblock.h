...
/*
 * $Id: humarobotics-wifiblock.h 112 2012-10-04 17:44:38Z xander $
 */
...
bool HRWBdoGET(tSensors link, tGetRequest &getrequest) {
  ...
  memcpy(HRWB_scratch, tmpPort, 2);
	...
	if (!HRWBwriteReg(link, HRWB_GET_URL, (tHugeByteArray)getrequest.URL[0], strsize(getrequest.URL[0]) + 1))
	  return false;
...
bool HRWBdoPOST(tSensors link, tPostRequest &postrequest) {
  ...
  memcpy(HRWB_scratch, tmpPort, 2);
	...
	if (!HRWBwriteReg(link, HRWB_POST_URL, (tHugeByteArray)postrequest.URL[0], strsize(postrequest.URL[0]) + 1))
	  return false;
