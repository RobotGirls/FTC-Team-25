...
/*
 * $Id: benedettelli-nxt2wifi-test1.c 112 2012-10-04 17:44:38Z xander $
 */
...
{
  ubyte length = N2Wrxbuffer[1];
  ubyte snmpversion = N2Wrxbuffer[4] + 1;
  string community;
  ubyte communityLength = N2Wrxbuffer[6];
  memcpy(community, &N2Wrxbuffer[7], communityLength);

  ubyte reqTypeOffset = 7 + communityLength;
  ubyte reqType = N2Wrxbuffer[reqTypeOffset];

  ubyte reqIDOffset = reqTypeOffset + 4;
  reqID = ((long)(N2Wrxbuffer[reqIDOffset + 0] & 0xFF) << 24) +
               ((long)(N2Wrxbuffer[reqIDOffset + 1] & 0xFF) << 16) +
               ((long)(N2Wrxbuffer[reqIDOffset + 2] & 0xFF) <<  8) +
               ((long)(N2Wrxbuffer[reqIDOffset + 3] & 0xFF) <<  0);

  ubyte error = N2Wrxbuffer[reqIDOffset + 6];
  ubyte errorIdx = N2Wrxbuffer[reqIDOffset + 9];

  ubyte oidOffset = reqIDOffset + 15;
  ubyte oidSize = N2Wrxbuffer[oidOffset];
  //ubyte oid[9];
  memcpy(oid, &N2Wrxbuffer[oidOffset + 1], oidSize);
  ubyte valueOffset = oidOffset + oidSize;
  ubyte valueType = N2Wrxbuffer[valueOffset];
  ubyte valueSize = N2Wrxbuffer[valueOffset + 1];
  string valueString;
  ...
  switch(valueType)
  {
    ...
    case ASN_OCTET_STRING:
      memcpy(valueString, &N2Wrxbuffer[valueOffset + 2], valueSize);
      break;
    case ASN_NULL: break;
  }
