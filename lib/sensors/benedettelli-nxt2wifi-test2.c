...
/*
 * $Id: benedettelli-nxt2wifi-test2.c 112 2012-10-04 17:44:38Z xander $
 */
...
void processData()
{
  ...
  for (int i = 0; i < sizeOfReceivedData; i++)
  {
    ...
    if (N2Wrxbuffer[i] > 0x20)
    {
      // Append to the current word
      strncat(singleword, &N2Wrxbuffer[i], 1);
    }
    // we've reached a word or line boundary
    else if ((N2Wrxbuffer[i] == 0x20) || (N2Wrxbuffer[i] == 0x0A))
    {
...
task main ()
{
  //string BOFHserver = "94.142.241.111"; // towel.blinkenlights.nl
    ...
    if (N2WTCPOpenClient(1, BOFHserver, BOFHport)) {
      ...
      // How many bytes are available in the buffers?
	    ...
	    if (avail > 0)
	    {
	      ...
	      {
	        writeDebugStream("%c", N2Wrxbuffer[i]);
	      }
